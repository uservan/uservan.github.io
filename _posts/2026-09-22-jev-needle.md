...
> - Practical takeaway for agent harnesses: compile the current state before calling Jev; do not hand it a raw history.
...
4. **This is a division-of-labor result, not a defect.** Jev is built for immediate judgment. The test says: whatever needs history to be merged must be merged *before* Jev sees it.
